/*
 * 4_atomics/ldadd/bench.c — STANDALONE paired LSE compare-and-swap benchmark (UNCONTENDED).
 *
 * Treatment: `cas` under a memory ORDER vs the RELAXED atomic (baseline). The
 ...
 * -march=native emits LSE cas/casa/casl/casal (objdump-verified); seqcst
...
static uint64_t op_##NAME(bb_workset_t *w, uint64_t iters, uint64_t stores, uint64_t *nout) { \
    ...
    for (uint64_t i = 0; i < iters; i++)                                       \
        for (uint64_t k = 0; k < stores; k++) {                               \
            uint64_t *p=ADDR(i*stores+k); uint64_t e=*p; __atomic_compare_exchange_n(p,&e,e+1u,0,MO,__ATOMIC_RELAXED); acc+=e; n++;         \
        }                                                                      \
    *nout = n; return acc;                                                     \
}
