/*
 * 4_atomics/ldadd/bench.c — STANDALONE paired LSE atomic-exchange benchmark (UNCONTENDED).
 *
 * Treatment: `swp` under a memory ORDER vs the RELAXED atomic (baseline). The
 ...
 * -march=native emits LSE swp/swpa/swpl/swpal (objdump-verified); seqcst
...
static uint64_t op_##NAME(bb_workset_t *w, uint64_t iters, uint64_t stores, uint64_t *nout) { \
    ...
    for (uint64_t i = 0; i < iters; i++)                                       \
        for (uint64_t k = 0; k < stores; k++) {                               \
            acc += __atomic_exchange_n(ADDR(i*stores+k), (uint64_t)(i*stores+k), MO); n++;         \
        }                                                                      \
    *nout = n; return acc;                                                     \
}
