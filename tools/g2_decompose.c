/* tools/g2_decompose.c — VERIFICATION probe for the G2 "hit/after_group Δ→0 at large N" claim.
 * Replicates 2_load_side/dmb_ishld/bench.c's EXACT load loop, but emits BOTH base and treat
 * full PMU (cyc, instructions, mem_access, stall_be_mem) per op so Δcyc can be decomposed into
 * (extra instructions = codegen) vs (extra stall/lost-overlap = the ordering effect).
 * usage: g2_decompose --placement after_group|after_every --stores N --iters I --repeats R
 *        [--ws BYTES] [--core C]   (hit = small resident ws, warmed) */
#define _GNU_SOURCE
...
static uint64_t pass_treat(bb_workset_t *w, uint64_t iters, uint64_t stores, int pl, uint64_t *nout){
    uint64_t acc=0,n=0; char *base=(char*)w->buf.base;
    if(pl==PLACE_EVERY){ for(uint64_t i=0;i<iters;i++) for(uint64_t k=0;k<stores;k++){ LOAD_ONE dmb_ishld(); } }
    else { for(uint64_t i=0;i<iters;i++){ for(uint64_t k=0;k<stores;k++){ LOAD_ONE } dmb_ishld(); } }
    *nout=n; return acc; }
...
int main(int argc,char**argv){
    ...
    sink += pass_base(&w, 200000, stores, &dummy);                 /* warmup (resident -> hit) */
    sink += pass_treat(&w, 200000, stores, pl, &dummy);
    bb_pmu_t pmu; if(bb_pmu_open(&pmu)!=0){fprintf(stderr,"PMU open fail\n");return 2;}
    double *bc=malloc(R*sizeof(double)),*bi=malloc(R*sizeof(double)),*bm=malloc(R*sizeof(double)),*bs=malloc(R*sizeof(double));
    double *tc=malloc(R*sizeof(double)),*ti=malloc(R*sizeof(double)),*tm=malloc(R*sizeof(double)),*ts=malloc(R*sizeof(double));
    double *bl=malloc(R*sizeof(double)),*tl=malloc(R*sizeof(double));
    ...
    printf("place=%-11s N=%-3lu | BASE cyc/op=%.3f ins/op=%.3f mem/op=%.3f l1/op=%.4f stall=%.1f%% "
           "| TREAT cyc/op=%.3f ins/op=%.3f mem/op=%.3f l1/op=%.4f stall=%.1f%% "
           "| Δcyc/op=%.3f Δins/op=%.3f Δmem/op=%.3f (sink=%lu)\n",
        pl_s,(unsigned long)stores,
    ...
    free(bc);free(bi);free(bm);free(bs);free(tc);free(ti);free(tm);free(ts);free(bl);free(tl);
    return 0;
}
