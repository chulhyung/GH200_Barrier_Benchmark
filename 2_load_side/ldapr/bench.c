/* 3_release_acquire/ldar — STANDALONE paired load-acquire (ldar vs ldr, RCpc weaker).
 * after_every = all ldar; after_group = N-1 ldr + 1 ldar. Paired baseline(ldr). */
#include "bench_common.h"
#include "aarch64_ops.h"
#define TREAT_NAME  "ldapr"
#define TREAT_LOAD(p) load_acquire_rcpc_64(p)
...
static uint64_t pass_base(bb_workset_t *w, uint64_t iters, uint64_t stores, uint64_t *nout){
    ...
    for(uint64_t i=0;i<iters;i++) for(uint64_t k=0;k<stores;k++){ acc ^= load_plain_64(ADDR(i*stores+k)); n++; }
    *nout=n; return acc; }
static uint64_t pass_treat(bb_workset_t *w, uint64_t iters, uint64_t stores, int placement, uint64_t *nout){
    uint64_t acc=0,n=0; char*base=(char*)w->buf.base;
    if(placement==PLACE_EVERY){ for(uint64_t i=0;i<iters;i++) for(uint64_t k=0;k<stores;k++){ acc ^= TREAT_LOAD(ADDR(i*stores+k)); n++; } }
    else { for(uint64_t i=0;i<iters;i++){ uint64_t k; for(k=0;k+1<stores;k++){ acc ^= load_plain_64(ADDR(i*stores+k)); n++; } acc ^= TREAT_LOAD(ADDR(i*stores+(stores-1))); n++; } }
    *nout=n; return acc; }
#define CSV_HEADER \
